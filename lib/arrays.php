...
class Arrays
{
     ...
     * @throws \ReflectionException
     */
    public static function data_get($target, ?string $key = null, $default = null)
    {
        if (is_null($key)) {
            return $target;
        }

        $target = arrayable($target) ? $target->toArray() : $target;
    ...
    {
        $reverseArgs = array_reverse($args);

        return array_merge(
            array_intersect(...$args),
            array_diff(...$args),
            array_diff(...$reverseArgs)
        );
    }
}
